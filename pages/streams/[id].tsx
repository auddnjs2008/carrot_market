    ...
    mutate(prev => prev && ({
      ...prev, stream: {
        ...
        messages: [...prev.stream.messages, {
          id: Date.now(), message: form.message,
          user: {
            ...user,
          } as any
        }]
      ...
      <div className="py-10 px-4  space-y-4">
        ...
        <div className="mt-5">
          ...
          <div className="bg-orange-300 p-5 rounded-md  overflow-scroll flex flex-col space-y-3">
            ...
            <span className="text-gray-600"><span className="font-medium text-gray-800">URL</span> {data?.stream.cloudflareUrl}</span>
            <span className="text-gray-600"><span className="font-medium text-gray-800">Key</span> {data?.stream.cloudflareKey}</span>
          </div>
        </div>
        <div>
          <h2 className="text-2xl font-bold text-gray-900">Live Chat</h2>
          <div className="py-10 pb-16 h-[50vh] overflow-y-scroll  px-4 space-y-4">
            {data?.stream.messages.map(message => <Message key={message.id} message={message.message} reversed={message.user.id === user?.id} />)}
          </div>
